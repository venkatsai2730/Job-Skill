  ...
  return (
    <section className="relative min-h-screen pt-28 pb-20 px-4 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 rounded-full px-5 py-2 border border-border bg-white/70">
            ...
            <span className="text-gray-600 text-sm">Trusted by <span className="text-foreground font-medium">100,000+</span> professionals</span>
          </div>
        </motion.div>
        ...
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
          className="text-gray-600 text-lg sm:text-xl max-w-2xl mx-auto mb-10 leading-relaxed text-center"
        >
        ...
        >
          ...
          <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
            <a href="/#how-it-works"
              className="flex items-center gap-2 border border-border text-gray-600 hover:text-foreground hover:border-border font-medium text-base px-8 py-4 rounded-full transition-all hover:bg-white/70">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
              <motion.div key={stat.label}
                ...
                className="text-center p-5 rounded-2xl border border-border bg-white/70 bento-glow"
              >
        ...
        >
          <p className="text-gray-400 text-xs uppercase tracking-widest mb-5">Professionals who landed roles at</p>
          <div className="flex items-center justify-center flex-wrap gap-8 sm:gap-12">
            {trustedLogos.map((logo) => (
              <span key={logo} className="text-gray-400 font-display font-bold text-sm sm:text-base opacity-40 hover:opacity-80 transition-opacity duration-300">
                {logo}
              </span>
            ))}
          </div>
