    ...
    <section id="faq" className="py-28 px-4 relative">
      ...
      <div className="max-w-3xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <p className="text-gray-600 text-lg">Everything you need to know about JobSkill AI.</p>
        </motion.div>
        <div className="space-y-3">
          {faqs.map((faq, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}
              className="rounded-2xl border border-border bg-white/70 overflow-hidden hover:border-purple-500/15 transition-colors duration-300">
