  ...
  <footer className="relative border-t border-border py-16 px-4">
    <div className="max-w-6xl mx-auto">
        ...
        <ScrollReveal direction="up" delay={0.1}>
          ...
          <ul className="space-y-2.5">
            {["AI Assistant", "ATS Scanner", "Job Tracker", "Resume Builder", "LinkedIn Optimizer"].map((item) => (
              <li key={item}>
                <Link to="/auth" className="text-gray-500 hover:text-foreground text-sm transition-colors">{item}</Link>
              </li>
            ))}
          </ul>
        </ScrollReveal>
        ...
        <ScrollReveal direction="up" delay={0.2}>
          ...
          <ul className="space-y-2.5">
            {["Blog", "Documentation", "Career Tips", "API"].map((item) => (
              <li key={item}><span className="text-gray-500 text-sm cursor-pointer hover:text-foreground transition-colors">{item}</span></li>
            ))}
          </ul>
        </ScrollReveal>
      ...
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}
        ...
        <p className="text-gray-400 text-xs">© 2026 JobSkill AI · 256-bit Encrypted · SOC2 Ready</p>
      </motion.div>
    </div>
  </footer>
);
