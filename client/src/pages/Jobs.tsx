...
function WhyThisJob({ job, userSkills }: { job: JobListing; userSkills: string[] }) {
  ...
  return (
    <div className="relative inline-block">
      <button onClick={() => setOpen(!open)} className="text-white-30 hover:text-cyan-spark transition-colors p-1 rounded" title="Why this job?">
        <Info className="w-3.5 h-3.5" />
      </button>
      {open && (
        <div className="absolute bottom-8 left-0 z-50 w-56 bg-surface-2 border border-white/[0.1] rounded-xl p-3 shadow-xl text-[11px]" onMouseLeave={() => setOpen(false)}>
          <p className="font-bold text-white-80 mb-1.5">Why this job?</p>
          ...
          <p className="text-white-40 mb-1">📊 {expFit}</p>
          {(job.location || "").toLowerCase().includes("remote") && <p className="text-blue-400">🌍 Remote position</p>}
          {job.match_score && job.match_score > 0 && <p className="text-cyan-spark mt-1">🏆 {job.match_score}% match score</p>}
        </div>
      )}
    </div>
  );
...
function CategorySection({ title, jobs, onSeeAll }: { title: string; jobs: JobListing[]; onSeeAll: () => void }) {
  if (jobs.length === 0) return null;
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-foreground">{title}</h3>
        <button onClick={onSeeAll} className="text-blue-electric text-[13px] font-semibold hover:underline flex items-center gap-1">
          See all <ChevronRight className="w-4 h-4" />
        </button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {jobs.slice(0, 4).map(job => <MiniJobCard key={job.id} job={job} />)}
      </div>
    </div>
  );
}

// ── Mini Card for Category Sections ───────────────────────
function MiniJobCard({ job }: { job: JobListing }) {
  return (
    <a href={job.job_url} target="_blank" rel="noreferrer" className="glass-card-hover p-4 flex flex-col gap-2 group cursor-pointer">
      <div className="flex items-center gap-2.5">
        <div className="w-9 h-9 rounded-lg bg-blue-electric/15 flex items-center justify-center text-blue-electric font-bold text-sm uppercase shrink-0">{job.company.charAt(0)}</div>
        <div className="min-w-0">
          <h4 className="text-sm font-semibold text-foreground truncate group-hover:text-blue-electric transition-colors">{job.title}</h4>
          <p className="text-white-40 text-[12px] truncate">{job.company}</p>
        </div>
      </div>
      <div className="flex items-center gap-2 flex-wrap">
        <SeniorityBadge level={job.seniority_level} />
        <span className="text-white-30 text-[11px] flex items-center gap-1"><MapPin className="w-3 h-3" /> {(job.location || "Remote").substring(0, 25)}</span>
      </div>
    </a>
  );
}
...
export default function Jobs() {
  ...
  const techJobs = feedJobs.filter(j => ["Software Development", "Data & Analytics", "DevOps & Cloud"].includes(j.category || ""));
  const fresherJobs = feedJobs.filter(j => (j.category === "Internships & Fresher") || (j.seniority_level === "intern" || j.seniority_level === "entry"));
  const remoteJobs = feedJobs.filter(j => (j.location || "").toLowerCase().includes("remote"));
  const topCompanyJobs = feedJobs.filter(j => ["google", "microsoft", "amazon", "flipkart", "swiggy", "zomato", "razorpay", "cred"].some(c => (j.company || "").toLowerCase().includes(c)));
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-1 flex flex-col">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8 w-full">
          <TabsList className="bg-surface-1 border border-white/[0.06] p-1 h-auto rounded-full w-full max-w-sm">
            <TabsTrigger value="feed" className="rounded-full px-6 py-2.5 text-[14px] data-[state=active]:bg-blue-electric data-[state=active]:text-white">Real-time Feed</TabsTrigger>
        ...
        <TabsContent value="feed" className="flex-1 mt-0 outline-none flex flex-col">
          ...
          <div className="bg-surface-1 border border-white/[0.06] rounded-2xl p-5 mb-8 flex flex-col lg:flex-row gap-4 lg:items-end w-full shadow-lg">
            ...
            <div className="w-full lg:w-[220px] space-y-1.5">
              ...
              <div className="relative">
                ...
                <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. Hyderabad, Remote"
                  ...
                  onKeyDown={(e) => e.key === "Enter" && handleApplyFilters()} />
                <button onClick={handleDetectLocation} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-white-30 hover:text-blue-electric transition-colors p-1 rounded" title="Detect my location">
            ...
            <button onClick={handleApplyFilters} className="w-full lg:w-auto bg-blue-electric hover:bg-blue-bright text-white font-semibold py-2.5 px-6 rounded-xl transition-all shadow-md shadow-blue-electric/20 active:scale-95 flex items-center justify-center">Search Jobs</button>
          </div>

          {loadingFeed ? (
            <div className="flex items-center justify-center py-20"><div className="animate-spin w-8 h-8 border-2 border-blue-electric border-t-transparent rounded-full" /></div>
          ) : feedJobs.length === 0 ? (
            <div className="text-center py-20 bg-surface-1 border border-white/[0.06] rounded-2xl">
              ...
              <button onClick={() => { setSearch(""); setLocation(""); setExperience(""); setSkills(""); fetchFeedJobs(true, 1, undefined, undefined, undefined, { query: "", location: "", experience: "", skills: "" }); }} className="mt-4 text-blue-electric hover:underline text-sm font-medium">Clear Filters</button>
            </div>
          ) : (
            <>
              {/* Category Sections */}
                ...
                {(feedSubTab === "for-you"
                  ...
                  <div key={job.id} className="glass-card-hover p-5 flex flex-col group relative">
                    ...
                    <div className="absolute top-4 right-4 flex items-center gap-1.5">
                      ...
                      <span className={`text-white-60 text-[10px] font-bold px-2 py-1 rounded capitalize tracking-wider flex items-center gap-1 ${job.source === "jsearch" ? "bg-emerald-500/15 text-emerald-400" : "bg-surface-3"}`}>
                        {job.source === "jsearch" ? "🌐 Global" : job.source}
                      </span>
                    </div>

                    <div className="flex items-center gap-3.5 mb-3 mt-2">
                      <div className="w-12 h-12 rounded-xl bg-blue-electric/15 flex items-center justify-center text-blue-electric font-display font-bold text-lg uppercase shrink-0">{job.company.charAt(0)}</div>
                    ...
                    <div className="flex flex-col gap-2 mb-5 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-white-60 text-[13px] font-medium">
                        <DollarSign className="w-4 h-4 shrink-0 text-white-30" /> <span className="truncate">{formatSalary(job.salary_min, job.salary_max)}</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5 mt-2">
                        {job.skills.slice(0, 3).map(skill => (
                          <span key={skill} className="bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded text-[11px] text-white-60">{skill}</span>
                        ))}
                        {job.skills.length > 3 && <span className="bg-white/[0.04] border border-white/[0.06] px-2 py-0.5 rounded text-[11px] text-white-40">+{job.skills.length - 3}</span>}
                        {job.skills.length === 0 && <span className="text-white-30 italic text-[11px]">No specific skills tagged</span>}
                      </div>
                    </div>

                    <div className="flex items-center justify-between border-t border-white/[0.06] pt-4 mt-auto">
                      <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1.5 text-white-40 text-[11px] font-medium"><Clock className="w-3.5 h-3.5" /> {formatTimeAgo(job.posted_at)}</div>
                        <WhyThisJob job={job} userSkills={userSkills} />
                      </div>
                      <div className="flex items-center gap-2.5">
                        <button onClick={() => saveToTracked(job)} className="text-white-60 hover:text-cyan-spark transition-colors flex items-center justify-center p-2 rounded-lg hover:bg-cyan-spark/10 outline-none" title="Save to Tracked Apps">
                          <BookmarkPlus className="w-4.5 h-4.5" />
                        </button>
                        <button onClick={() => { const prompt = `Analyze my fit for the ${job.title} role at ${job.company}. Skills: ${job.skills.join(", ")}.`; navigate(`/dashboard/chat?prompt=${encodeURIComponent(prompt)}`); }}
                          className="text-white-60 hover:text-purple-400 transition-colors flex items-center justify-center p-2 rounded-lg hover:bg-purple-400/10 outline-none" title="Match Score via AI">
                          <Bot className="w-4.5 h-4.5" />
                        </button>
                        <a href={job.job_url} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-blue-electric font-bold text-[13px] hover:text-white group/link outline-none transition-colors px-4 py-2 bg-blue-electric/10 hover:bg-blue-electric rounded-lg">
              ...
              {Math.ceil(totalJobs / 20) > 1 && (
                <div className="flex justify-center items-center gap-3 mt-12 mb-6">
                  <button onClick={() => handlePageChange(page - 1)} disabled={page === 1} className="flex items-center gap-1 bg-surface-2 border border-white/[0.06] hover:bg-surface-3 transition-colors px-4 py-2 rounded-lg text-sm font-semibold text-white-80 disabled:opacity-50 disabled:cursor-not-allowed outline-none">Previous</button>
                    ...
                    {Array.from({ length: Math.min(5, Math.ceil(totalJobs / 20)) }, (_, i) => {
                      ...
                      return (
                        <button key={pageNum} onClick={() => handlePageChange(pageNum)}
                          className={`w-9 h-9 rounded-lg flex items-center justify-center text-[13px] font-bold transition-all outline-none ${page === pageNum ? "bg-blue-electric text-white shadow-lg shadow-blue-electric/20" : "bg-surface-2 border border-white/[0.06] text-white-60 hover:bg-surface-3 hover:text-white"}`}>
                          {pageNum}
                        </button>
                      );
                    })}
                  </div>
                  <button onClick={() => handlePageChange(page + 1)} disabled={page === Math.ceil(totalJobs / 20)} className="flex items-center gap-1 bg-surface-2 border border-white/[0.06] hover:bg-surface-3 transition-colors px-4 py-2 rounded-lg text-sm font-semibold text-white-80 disabled:opacity-50 disabled:cursor-not-allowed outline-none">Next</button>
                </div>
              )}
            </>
          )}
                ...
                return (
                  <div key={col.name} className="flex-1 min-w-[300px] w-full">
                    ...
                    <div className="space-y-4">
                        ...
                        <div key={job.id} className="glass-card-hover p-5 cursor-pointer group relative">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3.5 min-w-0 pr-4">
                              <div className="w-10 h-10 rounded-xl bg-blue-electric/15 flex items-center justify-center text-blue-electric font-display font-bold text-base uppercase shrink-0">{job.company.charAt(0)}</div>
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-2 mb-5">
                            <div className="flex items-center gap-1.5 text-white-40 text-[12px] font-medium"><MapPin className="w-3.5 h-3.5 text-white-30" /> <span className="truncate">{job.location}</span></div>
                            <div className="flex items-center gap-1.5 text-white-40 text-[12px] font-medium"><DollarSign className="w-3.5 h-3.5 text-white-30" /> <span className="truncate">{job.salary}</span></div>
                          </div>
                          <div className="flex items-center justify-between mb-5">
                            ...
                            <div className="flex items-center gap-1.5 text-white-30 text-[11px] font-medium"><Clock className="w-3 h-3" /> {formatTimeAgo(job.created_at)}</div>
                          </div>
                          <div className="flex justify-between items-center border-t border-white/[0.06] pt-4 mt-1">
                            ...
                            <a href={job.job_url || "#"} target={job.job_url ? "_blank" : "_self"} rel="noreferrer"
                              ...
                              className="flex items-center gap-1.5 text-white-40 font-medium text-[13px] hover:text-white-60 outline-none transition-colors">
                      ...
                      {colJobs.length === 0 && (
                        <div className="border border-dashed border-white/[0.08] rounded-2xl p-6 text-center text-white-30 text-[13px] font-medium flex items-center justify-center min-h-[120px]">Drop jobs here</div>
                      )}
                    </div>
                  </div>
                );
