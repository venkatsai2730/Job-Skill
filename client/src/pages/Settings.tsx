...
const SettingsPage = () => {
  ...
  const cardCls = "bg-surface-1 border border-white/[0.06] rounded-2xl p-8 shadow-sm";
  const inputCls = "w-full bg-surface-2 border border-white/[0.06] rounded-xl px-4 py-3 text-foreground text-base outline-none focus:border-blue-electric focus:ring-2 focus:ring-blue-electric/10 transition-all placeholder:text-white-30";
          ...
          <nav className="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${activeTab === tab.id ? "bg-blue-electric/10 text-blue-electric shadow-sm" : "text-white-60 hover:bg-surface-2 hover:text-foreground"}`}>
                  ...
                  <div className={cardCls}>
                    ...
                    <div className="flex items-center gap-6 mb-8">
                      <div className="relative">
                        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-electric to-cyan-spark flex items-center justify-center text-white font-display font-bold text-2xl shadow-lg shadow-blue-electric/20">{userInitials}</div>
                        <button className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-surface-2 border border-white/[0.06] shadow-sm flex items-center justify-center hover:bg-surface-3 transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-blue-electric text-sm mt-1">{(user?.plan || "free").charAt(0).toUpperCase() + (user?.plan || "free").slice(1)} Plan</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-6 pt-6 border-t border-white/[0.06]">
                      <button onClick={handleProfileSave} disabled={profileSaving} className="bg-blue-electric hover:bg-blue-bright text-white px-8 py-3 rounded-xl font-medium text-sm transition-all hover:shadow-lg hover:shadow-blue-electric/20 disabled:opacity-50">
                        {profileSaving ? "Saving..." : "Save Changes"}
                      </button>
                      <button onClick={() => setDisplayName(user?.fullName || "")} className="text-white-60 hover:text-foreground px-6 py-3 rounded-xl font-medium text-sm transition-colors">Cancel</button>
                    </div>
                  </div>
                  <div className={cardCls}>
                    ...
                    <p className="text-white-30 text-sm mt-2">142 / 300 characters</p>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-6">
                  <div className={cardCls}>
                    <div className="flex items-start justify-between mb-6">
                      ...
                      <span className="bg-blue-electric/10 text-blue-electric text-sm px-4 py-1.5 rounded-full font-semibold border border-blue-electric/20">Free</span>
                    </div>
                    <div className="mb-6">
                    ...
                    <Link to="/pricing" className="inline-flex items-center gap-2 bg-blue-electric hover:bg-blue-bright text-white px-8 py-3 rounded-xl font-medium text-sm transition-all hover:shadow-lg hover:shadow-blue-electric/20">
                  ...
                  <div className={cardCls}>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {[
                        ...
                        <div key={plan.name} className={`rounded-xl p-5 border-2 ${plan.name === "Pro" ? "border-blue-electric bg-blue-electric/5" : "border-white/[0.06]"}`}>
                          ...
                          <ul className="space-y-2">
                            {plan.features.map((f) => (<li key={f} className="flex items-center gap-2 text-sm text-white-60"><Check className="w-4 h-4 text-blue-electric shrink-0" /> {f}</li>))}
                          </ul>
                          <button className={`w-full mt-5 py-2.5 rounded-lg font-medium text-sm transition-all ${plan.current ? "bg-surface-2 text-white-60 cursor-default" : plan.name === "Pro" ? "bg-blue-electric hover:bg-blue-bright text-white" : "border border-white/[0.06] text-foreground hover:bg-surface-2"}`}>
              ...
              {activeTab === "api" && (
                <div className="space-y-6">
                  <div className={cardCls}>
                    ...
                    <div className="bg-surface-2 border border-white/[0.06] rounded-xl p-5 mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        <code className="flex-1 bg-surface-3 border border-white/[0.06] rounded-lg px-4 py-2.5 text-sm font-mono text-foreground">
                          {showKey ? "sk_live_a8f7g6h5j4k3l2m1n0o9p8q7r6" : "sk_live_••••••••••••••••••••••••••••"}
                        </code>
                        <button onClick={() => setShowKey(!showKey)} className="p-2.5 rounded-lg border border-white/[0.06] hover:bg-surface-2 transition-colors">
                          {showKey ? <EyeOff className="w-4 h-4 text-white-60" /> : <Eye className="w-4 h-4 text-white-60" />}
                        </button>
                        <button onClick={handleCopy} className="p-2.5 rounded-lg border border-white/[0.06] hover:bg-surface-2 transition-colors">
                          {copiedKey ? <Check className="w-4 h-4 text-emerald-400" /> : <Copy className="w-4 h-4 text-white-60" />}
                        </button>
                    ...
                    <button className="bg-blue-electric hover:bg-blue-bright text-white px-6 py-3 rounded-xl font-medium text-sm transition-all flex items-center gap-2">
                  ...
                  <div className={cardCls}>
                    ...
                    <div className="bg-surface-3 rounded-xl p-5 overflow-x-auto">
                      <pre className="text-cyan-spark text-sm font-mono">{`curl -X POST https://api.jobskill.ai/v1/ats-scan \\
  ...
  -d '{"resume_url": "https://..."}'`}</pre>
                    ...
                    <div className="divide-y divide-white/[0.06]">
                      {[
                        ...
                        <div key={n.label} className="flex items-center justify-between py-5">
                          ...
                          <div className={`w-12 h-6 rounded-full cursor-pointer transition-colors shrink-0 ml-6 ${n.enabled ? "bg-blue-electric" : "bg-surface-3"}`}>
              ...
              {activeTab === "security" && (
                <div className="space-y-6">
                  <div className={cardCls}>
                    ...
                    <div className="space-y-4 max-w-md">
                      ...
                      <button onClick={handleChangePassword} disabled={passwordSaving || !currentPassword || !newPassword || !confirmPassword}
                        className="bg-blue-electric hover:bg-blue-bright text-white px-8 py-3 rounded-xl font-medium text-sm transition-all hover:shadow-lg hover:shadow-blue-electric/20 mt-2 disabled:opacity-50">
                    ...
                    <div className="space-y-4">
                      {[
                        ...
                        <div key={account.name} className="flex items-center justify-between py-3 px-4 bg-surface-2 rounded-xl">
                          ...
                          <button className={`text-sm font-medium px-4 py-2 rounded-lg transition-colors ${account.connected ? "text-red-400 hover:bg-red-500/10" : "text-blue-electric hover:bg-blue-electric/10"}`}>
                  ...
                  <div className="bg-surface-1 border border-red-500/20 rounded-2xl p-8 shadow-sm">
                  ...
                  <div className={cardCls}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {[
                        ...
                        <div key={item.fmt} className="bg-surface-2 rounded-xl p-6 text-center border border-white/[0.06] hover:border-blue-electric/40 transition-colors cursor-pointer">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {["Profile information", "Resume content", "Job applications", "AI chat history", "Skill assessments", "LinkedIn suggestions"].map((item) => (
                        <div key={item} className="flex items-center gap-2.5 text-sm text-foreground">
                          <Check className="w-4 h-4 text-blue-electric shrink-0" /> {item}
                        </div>
                      ))}
                    </div>
                  </div>
