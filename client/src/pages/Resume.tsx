  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <div className="w-56 shrink-0 flex flex-col border-r border-border/60 bg-surface-1 p-4 overflow-y-auto">
        ...
        {/* Section nav — always visible when a file is loaded */}
        {resumeFile && (
          <nav className="space-y-0.5 flex-1">
            {SECTIONS.map((s) => (
              <button type="button" key={s} onClick={() => scrollToSection(s)}
                className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${activeSection === s ? "bg-blue-electric/10 text-blue-electric border-l-[3px] border-blue-electric" : "text-white-60 hover:bg-surface-2 hover:text-foreground"}`}
              >{s}</button>
            ))}

            {/* AI hint — compact, shown only before first AI edit */}
            {!hasAIEdits && (
              <div className="mt-3 px-2 py-3 rounded-lg bg-surface-2/50 flex items-start gap-2">
                <Sparkles className="w-3.5 h-3.5 text-blue-electric mt-0.5 shrink-0" />
                <p className="text-white-30 text-xs leading-relaxed">Type a prompt below to update with AI</p>
              </div>
            )}
          </nav>
        )}
        ...
        <div className="space-y-2 mt-4 pt-3 border-t border-border/40">
          <button type="button" onClick={handleDownloadPDF}
            ...
            className="w-full bg-blue-electric hover:bg-blue-bright text-primary-foreground text-sm py-2.5 rounded-lg font-semibold flex items-center justify-center gap-1.5 disabled:opacity-40 transition-colors">
          ...
          <button type="button" onClick={handleDownloadDOCX} disabled={!hasParsed || downloading === "docx"}
            className="w-full border border-border/60 text-white-60 hover:text-foreground hover:border-border text-sm py-2.5 rounded-lg font-medium flex items-center justify-center gap-1.5 disabled:opacity-40 transition-colors">
      ...
      {/* ── Right panel ─────────────────────────────────── */}
      <div className="w-80 shrink-0 border-l border-border/60 bg-surface-1 overflow-y-auto p-5 space-y-5">
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() => {
              if (ats) {
                navigator.clipboard.writeText(`My resume ATS score: ${atsScore}/100 (${ats.label}) — JobSkill AI`);
                toast.success("Score copied to clipboard!");
              }
            }}
            disabled={!ats}
            className="flex-1 bg-blue-electric hover:bg-blue-bright text-primary-foreground text-sm py-2.5 rounded-lg font-semibold flex items-center justify-center gap-1.5 disabled:opacity-40 transition-colors"
          >
          ...
          <button
            ...
            disabled={!resumeFile || exporting}
            className="flex-1 border border-border/60 text-white-60 hover:text-foreground hover:border-border text-sm py-2.5 rounded-lg font-medium flex items-center justify-center gap-1.5 disabled:opacity-40 transition-colors"
          >
            ...
            <>
              ...
              <div className="mt-3 flex justify-center">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold ${
                  ats.atsRisk === "HIGH"   ? "bg-red-500/15 text-red-400 border border-red-500/30" :
                  ats.atsRisk === "MEDIUM" ? "bg-amber-500/15 text-amber-400 border border-amber-500/30" :
                                             "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30"
                }`}>
                  <AlertTriangle className="w-3 h-3" />
                  ATS RISK: {ats.atsRisk}
                </span>
              </div>
            </>
          ) : (
        ...
        {ats?.issues && ats.issues.length > 0 && (
          <div>
            ...
            <div className="space-y-2.5">
              {ats.issues.filter(i => i.type === "warning").slice(0, 7).map((issue) => (
                <div key={issue.text} className="flex items-start gap-2.5 text-sm">
                  <AlertTriangle className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />
                  <span className="text-foreground/80 text-xs leading-relaxed">{issue.text}</span>
                </div>
              ))}
              {ats.issues.filter(i => i.type === "success").slice(0, 3).map((issue) => (
                <div key={issue.text} className="flex items-start gap-2.5 text-sm">
                  <Check className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />
                  <span className="text-emerald-500/80 text-xs leading-relaxed">{issue.text}</span>
                </div>
              ))}
            </div>
          </div>
